...
function holi(num) {
  return new Promise((resolve, reject) => {
    if (num === 3) {
      reject(`ERROR ${num}} no debe ser igual a 3`);
    }
    resolve(num * 2);
  });
}
function holi2(num) {
  return new Promise((resolve, reject) => {
    if (num === 10) {
      reject(`ERROR ${num} no debe ser igual a 10`);
    }
    resolve(num * 4);
  });
}
...
const url = "https://jestjs.io/es-ES/docs/manual-mocks#mocking-user-modules";
const getStatus = () => {
...
fetch(url, { method: "GET" })
  ...
  .catch((err) => console.log(err));
//PRACTICA APLICACIÓN DE CONSOLA ------------------------
// const showMenu = () => {
